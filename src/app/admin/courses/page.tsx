...
const emptyForm = {
  ...
  level: "BEGINNER", status: "DRAFT", categoryId: "", featured: false, thumbnail: "",
};
...
export default function AdminCoursesPage() {
  ...
  const fetchCourses = useCallback(async (s: string, st: string, p: number) => {
    ...
    try {
      ...
      setStats({ total: data.total ?? 0, published: data.statusCounts?.PUBLISHED ?? 0, draft: data.statusCounts?.DRAFT ?? 0, archived: data.statusCounts?.ARCHIVED ?? 0 });
  ...
  function openEdit(c: Course) {
    setForm({ title: c.title, description: "", price: String(c.price), salePrice: c.salePrice ? String(c.salePrice) : "", level: c.level, status: c.status, categoryId: c.category.id, featured: c.featured, thumbnail: "" });
  ...
  async function handleFormSubmit(e: React.FormEvent) {
    ...
    try {
      const body = { ...form, title: form.title.trim(), price: Number(form.price) || 0, salePrice: form.salePrice ? Number(form.salePrice) : null };
      const res = modal === "add"
      ...
      setModal(null); fetchCourses(search, statusFilter, page);
  ...
  async function handleStatusToggle(c: Course) {
    ...
    try {
      const res = await fetch("/api/admin/courses", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: c.id, action }) });
      if (!res.ok) throw new Error();
  ...
  async function handleBulkAction(action: "publish" | "archive") {
    try {
      const res = await fetch("/api/admin/courses", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selected, action }) });
      if (!res.ok) throw new Error();
      setSelected([]); fetchCourses(search, statusFilter, page);
  ...
  async function handleBulkDelete() {
    ...
    try {
      const res = await fetch("/api/admin/courses", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selected }) });
      const data = await res.json();
      if (!res.ok) { alert(data.error ?? "Không thể xóa."); return; }
      setSelected([]); setDeleteConfirm(false); fetchCourses(search, statusFilter, page);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shadow-sm">
      ...
      {selected.length > 0 && (
        <div className="flex items-center gap-3 bg-teal-50 border border-teal-200 rounded-xl px-4 py-2.5">
          ...
          <div className="flex gap-2 ml-auto">
            <button onClick={() => handleBulkAction("publish")} className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-teal-600 text-white hover:bg-teal-700">Xuất bản</button>
            <button onClick={() => handleBulkAction("archive")} className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300">Lưu trữ</button>
            <button onClick={() => setDeleteConfirm(true)} className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-red-500 text-white hover:bg-red-600 flex items-center gap-1"><Trash2 className="h-3 w-3" />Xóa</button>
              ...
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="px-4 py-3">
                    <input type="checkbox" checked={allSelected} onChange={() => setSelected(allSelected ? [] : courses.map(c => c.id))} className="rounded border-gray-300 text-teal-600 focus:ring-teal-500" />
                  </th>
                ...
                {courses.map(c => (
                  <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <input type="checkbox" checked={selected.includes(c.id)} onChange={() => setSelected(prev => prev.includes(c.id) ? prev.filter(x => x !== c.id) : [...prev, c.id])} className="rounded border-gray-300 text-teal-600 focus:ring-teal-500" />
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${statusColors[c.status]}`}>{statusLabels[c.status]}</span>
                    </td>
                    <td className="px-3 py-3">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg my-4 z-10">
            ...
            <form onSubmit={handleFormSubmit} className="px-6 py-4 space-y-4">
              ...
              <div>
                ...
                <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} required className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Mô tả</label>
                <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={3} className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Danh mục *</label>
                <select value={form.categoryId} onChange={e => setForm({ ...form, categoryId: e.target.value })} required className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white">
                  <option value="">-- Chọn danh mục --</option>
                  {categories.map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
                </select>
                {categories.length === 0 && <p className="text-xs text-amber-500 mt-1">Chưa có danh mục. <a href="/admin/categories" className="underline">Tạo ngay</a></p>}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Giá gốc (₫)</label>
                  <input type="number" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} min="0" className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Giá khuyến mãi (₫)</label>
                  <input type="number" value={form.salePrice} onChange={e => setForm({ ...form, salePrice: e.target.value })} min="0" className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Cấp độ</label>
                  <select value={form.level} onChange={e => setForm({ ...form, level: e.target.value })} className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white">
                ...
                <div>
                  ...
                  <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white">
                    <option value="DRAFT">Nháp</option>
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="featured" checked={form.featured} onChange={e => setForm({ ...form, featured: e.target.checked })} className="rounded border-gray-300 text-teal-600 focus:ring-teal-500" />
                <label htmlFor="featured" className="text-sm text-gray-700">Đánh dấu nổi bật</label>
              </div>
              <div className="flex gap-3 pt-2">
                <button type="submit" disabled={formLoading} className="flex-1 py-2.5 bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
                ...
                <button type="button" onClick={() => setModal(null)} className="flex-1 py-2.5 border border-gray-200 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-50 transition-colors">Hủy</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 z-10 text-center">
            <div className="h-14 w-14 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4"><Trash2 className="h-6 w-6 text-red-500" /></div>
            <h2 className="text-lg font-bold text-gray-900 mb-2">Xác nhận xóa</h2>
            <p className="text-sm text-gray-500 mb-5">Xóa {selected.length} khóa học đã chọn?<br /><span className="text-red-500 font-medium">Không thể xóa nếu còn học viên.</span></p>
            <div className="flex gap-3">
              <button onClick={handleBulkDelete} disabled={deleteLoading} className="flex-1 py-2.5 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
